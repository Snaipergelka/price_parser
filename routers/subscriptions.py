...
async def subscribe_to_product(user: schemas.UserAuth,
                               product: schemas.ProductURLAndIDCreate,
                               conn: crud.CRUD = Depends(connecting_to_db)):

    logger.info(f"Accepted {user} to subscribe to {product}.")
    ...
    product_id = conn.get_product_by_url(url=product.url)
    ...
    if user and product_id:
        ...
        logger.info(f"Subscribed {user} to {product}.")
        return "Success"

    elif user and not product_id:

        logger.info(f"Parsing all the information about the {product}.")
        ...
        product = conn.create_product(product=product)
...
@router.delete("/unsubscribed_product")
